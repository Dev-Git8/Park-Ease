...
This is a full-stack **parking booking platform** — think of it like a mini "Booking.com for parking spots." There are three types of users: **customers** who search for parking and book a slot in advance, **business owners** who list their parking lots and manage available slots, and **admins** who approve new businesses and oversee the platform. A customer picks a time range for a specific slot, pays for it online through Razorpay, and the slot is locked just for that time window — the system supports multiple future bookings on the same slot as long as they don't overlap, similar to how a hotel room can be booked for different date ranges by different guests. If someone overstays past their booked time, the backend automatically calculates and charges a penalty. The stack is **Node.js/Express** for the API, **PostgreSQL with Prisma** as the ORM, **Redis** for caching and rate limiting, and **Socket.IO** for real-time updates (e.g., a slot instantly shows as "occupied" or "available" to everyone viewing that lot). The interesting engineering problems I solved here were: preventing double-booking under concurrent requests using database row locking (`SELECT ... FOR UPDATE` inside a transaction), making sure money/pricing is always calculated server-side (never trusted from the client) to prevent tampering, closing a privilege-escalation bug where anyone could register as an admin, and integrating a real payment gateway (migrated from Stripe to Razorpay since Stripe isn't available in India) with idempotent webhook handling so a payment is never double-processed. I also hardened it for production with rate limiting, input validation (Zod), CI, and Docker.
